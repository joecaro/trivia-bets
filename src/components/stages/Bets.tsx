...
export default function Bets() {
  ...
  const currentAnswers = useGameStore(
    (state) => state.currentAnswers,
    (a, b) => equal(a, b),
  );
  const currentQuestionIndex = useGameStore((state) =>
    state.currentQuestionIndex
  );
  const currentBets = useGameStore(
    (state) => state.currentBets,
    (a, b) => equal(a, b),
  );
  ...
  const userBets = useMemo(
    () => socket.id ? currentBets[socket.id] || defaultBets : defaultBets,
    [currentBets, socket],
  );
  ...
  return (
    <div className="flex flex-col gap-8 items-center justify-center">
      ...
      <div className="flex justify-center gap-5">
        <AnswerSpot
          key={"none"}
          label="Smaller than any answer"
          answer={"none"}
          onDrop={(betIdx) => bet("none", 5, betIdx)}
          odds={`5-1`}
        />
        {even
          ? (
            <>
              {answers.slice(0, middle).map(([userId, answer], i) => (
                <AnswerSpot
                  ...
                  onDrop={composeBet(answer.answer, payout(i))}
                  odds={`${(payout(i)).toString()}-1`}
                />
              ))}
              ...
              {answers.slice(middle).map(([userId, answer], i) => (
                <AnswerSpot
                  ...
                  userId={userId}
                  onDrop={(betIdx) =>
                    bet(answer.answer, payout(middle + i), betIdx)}
                  odds={`${(payout(middle + i)).toString()}-1`}
                />
              ))}
            </>
          )
          : answers.map(([userId, answer], i) => (
            <AnswerSpot
              ...
              answer={answer.answer}
              userId={userId}
              onDrop={(betIdx) => bet(answer.answer, payout(i), betIdx)}
              odds={`${(payout(i)).toString()}-1`}
            />
          ))}
